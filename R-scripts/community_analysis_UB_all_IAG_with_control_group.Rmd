...
ordiellipse(nms, ub_vegnorare$YEAR, col = 1:4, kind="ehull", label = TRUE, show.groups = c(16, 18, 19, 20))
...
funcdf$BURN_YR <- paste(funcdf$BURN17, funcdf$YEAR, sep = "_")
...
sp4nmsfunc <- funcdf[which(funcdf$YEAR == 16 | funcdf$YEAR == 20), 1:10]
...
siteScores_burnFunc <- cbind(data.frame(siteScoresFunc), funcdf_sub[, c("BURN2", "YEAR", "BURN17")])

speciesFunc <- rownames(speciesScoresFunc)
```

add environmental variable for pre-treatment canopy 2016
```{r}
# envdf <- func_sum[,c(1,11)]
# envdf$YEAR <- substr(envdf$PLOT_YR, 1,2)
# envdf$PLOT <- substr(envdf$PLOT_YR, 3, 8)
# 
# #subset for pre and post treatment
# envdf <- filter(envdf, grepl('16UB', PLOT_YR) | grepl('20UB', PLOT_YR) )
# 
# #create var for just 2016 tree cover 
# envdf16 <- filter(envdf, YEAR == 16)
# envdf16$Tree16 <- envdf16$Tree
# 
# envdf2 <- merge(envdf, envdf16[,4:5])
# 
# #fit to ordination
# env.fit <- envfit(nmsfunc_rot, envdf2, perm=999, scaling = "species")
## somehting wrong with these plots!
```
...
with(funcdf_sub, points(nmsfunc_rot, display = "sites", col = c("blue", "#52796f", "#dda15e", "#bc6c25")[BURN17], cex=1.25, pch = c(16, 17)[YEAR]))


ordiellipse(nmsfunc_rot, funcdf_sub$BURN_YR,
            scaling = 3, label = FALSE, draw = "polygon",
            col = c("#dda15e", "red",  "#52796f", "blue"),
            kind = "sd",
            alpha = 40,
...
#add environmental arrow for tree cover pre-treatment
# plot(env.fit) # something wrong with these plots!
...
func.mrpp16 <- with(funcdf_sub[which(funcdf_sub$YEAR == 16),], mrpp(sp4nmsfunc16, BURN2, distance = "euclidean"))
func.mrpp16

func.mrpp20 <- with(funcdf_sub[which(funcdf_sub$YEAR == 20),], mrpp(sp4nmsfunc20, BURN2, distance = "euclidean"))
func.mrpp20
...
funcmp.unburn <- with(funcdf_unburned, mrpp(funcdf_unburned[,2:10], YEAR, distance = "euclidean"))
funcmp.lowsev <- with(funcdf_lowsev, mrpp(funcdf_lowsev[,2:10], YEAR, distance = "euclidean"))
funcmp.highsev <- with(funcdf_highsev, mrpp(funcdf_highsev[,2:10], YEAR, distance = "euclidean"))
...
#explore means
func.md16 <- with(funcdf_sub[which(funcdf_sub$YEAR == 16),], meandist(vegdist(sp4nmsfunc16), BURN2, distance = "euclidean"))
summary(func.md16)

plot(func.md16, kind = "histogram")

func.md20 <- with(funcdf_sub[which(funcdf_sub$YEAR == 20),], meandist(vegdist(sp4nmsfunc20), BURN2, distance = "euclidean"))
summary(func.md20)
