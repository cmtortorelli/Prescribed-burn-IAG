...
sub = ub_veg[c("PLOT","YEAR","BURN17", "VEDU80", "BRJA80", "BRTE80", "CANOPY")]
#change year to factor
sub$YEAR <- as.factor(sub$YEAR)

#add brome col by adding brja and brte
sub$BROME <- sub$BRJA80 + sub$BRTE80

#add IAG col
sub$IAGall <- sub$BROME + sub$VEDU80
#add IAG percent cover col
sub$IAGperc <- sub$IAGall/80*100
...
lm2 <- polr(BURN17 ~ IAGperc + CANOPY, data = orddf, Hess = TRUE)
summary(lm2)
...
Attempting a multinomial model as it does not assume normality, linearity, or homoscedasticity
...
lrtest(m1,lm2) #multinomial model is a better fit than ordinal regression and assumptions are met
```
Decided to use a multinomial model because it fits the data best and the data meet all assumptions. 

Instead of calculating probabilities of transitions, we'll calculate the probability 
